...
final class ScreenshotService {
    var hotkey = ScreenshotHotkey()
    private var memoryStore: MemoryStore?

    func configure(memoryStore: MemoryStore) {
        self.memoryStore = memoryStore
    }

    func takeScreenshot() {
        guard let store = memoryStore else { return }
        store.ensureTodayFolder()
...
final class ScreenshotHotkey {
    ...
    private func loadFromDefaults() {
        ...
        isEnabled = d.bool(forKey: "noodl.screenshot.hotkey.enabled")
        modifierFlags = UInt(d.integer(forKey: "noodl.screenshot.hotkey.modifiers"))
        keyCode = UInt16(d.integer(forKey: "noodl.screenshot.hotkey.keyCode"))
    }

    private func saveToDefaults() {
        let d = UserDefaults.standard
        d.set(isEnabled, forKey: "noodl.screenshot.hotkey.enabled")
        d.set(Int(modifierFlags), forKey: "noodl.screenshot.hotkey.modifiers")
        d.set(Int(keyCode), forKey: "noodl.screenshot.hotkey.keyCode")
    }
